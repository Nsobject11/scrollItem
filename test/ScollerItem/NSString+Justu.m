...
//密码（8～12）位字符或者数字
- (BOOL)isPassword{
    NSString *emailRegex = @"^(?![0-9]+$)(?![a-zA-Z]+$)[a-zA-Z0-9]{6,20}";
...
- (BOOL)isCntrNumber {
    ...
    NSString *regex = @"^[a-zA-Z]{4}([0-9]){0,7}$";
    return [self isValidateByRegex:regex];
}


// UTC时间转成本地时间
-(NSString *)cos_utcTimeToLocalTime {
...
- (NSInteger )an_transformDateStrToStampWithFmt:(NSString *)fmtStr
{
    ...
    NSInteger totalStamp = interval * 1000 ;
//    NSLog(@"转换的时间戳: %ld",totalStamp);
...
{    
    ...
    NSDate *date = [fmt dateFromString:self];
    fmt.dateFormat = toFmt;
    return self.length ? [fmt stringFromDate:date] : @"";
}
...
{
    if ([self simpleVerifyIdentityCardNum]) {
